...
export const databaseManifest: CollectionManifest[] = [
  ...
  { name: "products", indexes: [{ key: { slug: 1 }, unique: true }, { key: { state: 1, categorySlug: 1, publishedAt: -1 } }, { key: { name: "text", brand: "text", blurb: "text" } }] },
  ...
  { name: "stockAlerts", indexes: [{ key: { email: 1, variantId: 1 }, unique: true }] },
  { name: "notifications", indexes: [{ key: { userId: 1, createdAt: -1 } }] },
  { name: "outbox", indexes: [{ key: { status: 1, nextAttemptAt: 1 } }, { key: { dedupeKey: 1 }, unique: true }] },
  { name: "newsletterSubscriptions", indexes: [{ key: { email: 1 }, unique: true }] },
  { name: "contactMessages", indexes: [{ key: { createdAt: -1 } }] },
  { name: "searchRequests", indexes: [{ key: { normalizedQuery: 1, createdAt: -1 } }] },
  { name: "auditLogs", indexes: [{ key: { actorId: 1, createdAt: -1 } }, { key: { entityType: 1, entityId: 1, createdAt: -1 } }] },
  { name: "idempotency", indexes: [{ key: { ownerKey: 1, key: 1, operation: 1 }, unique: true }, { key: { expiresAt: 1 }, expireAfterSeconds: 0 }] },
  { name: "jobs", indexes: [{ key: { state: 1, scheduledFor: 1 } }] },
